...
<div class="flex justify-end mb-4">
  <a href="{{ '/problems/' | relative_url }}" class="inline-flex items-center gap-2 px-3 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-bold rounded shadow-sm transition-colors border border-indigo-100 no-underline">
...
<div class="flex flex-col xl:flex-row gap-4">
  ...
  <div class="flex-1 min-w-0">
    <table class="w-full text-left border-collapse table-auto bg-white rounded-lg border border-slate-200 overflow-hidden">
      <thead>
        <tr class="border-b border-slate-200">
          <th class="px-3 py-2 text-[10px] font-extrabold text-slate-500 uppercase tracking-widest w-8 text-center">#</th>
          <th class="px-3 py-2 text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Title</th>
          <th class="px-3 py-2 text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Questions</th>
          <th class="px-3 py-2 text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Solutions</th>
        </tr>
      </thead>
      <tbody id="lectureBody">
        ...
        {% for p in lectures %}
          ...
          <tr class="lecture-row border-b border-slate-100 hover:bg-blue-50/20 transition-colors" 
            ...
            <td class="px-3 py-2 text-sm font-black text-slate-400 text-center">{{ lecture_num }}</td>
            <td class="px-3 py-2">
  ...
  <aside class="w-full xl:w-48 flex-shrink-0">
    <div class="flex items-center justify-between mb-2">
      <span class="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Tags</span>
    ...
    <div class="flex flex-wrap gap-1.5">
      ...
      {% for tag in tags %}
        <span class="tag-item px-2 py-0.5 bg-slate-50 text-slate-600 rounded text-[10px] font-medium" onclick="filterByTag('{{ tag }}', this)">{{ tag }}</span>
      {% endfor %}
    </div>
  </aside>

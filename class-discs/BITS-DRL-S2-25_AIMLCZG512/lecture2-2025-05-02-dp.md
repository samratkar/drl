...
$\epsilon$-greedy explores **blindly** — when it explores, it picks a random action regardless of uncertainty. UCB is smarter — it favors actions the agent is *less certain* about:

$$A_t = \argmax_a \left[ Q_t(a) + c \sqrt{\frac{\ln t}{N_t(a)}} \right]$$

- $N_t(a)$ = number of times action $a$ has been selected
- $c$ controls the degree of exploration (here $c = 2$)
- The square-root term is an **uncertainty bonus** — actions tried less often get a larger bonus

![Figure 2.4 — UCB (c=2) vs epsilon-greedy (epsilon=0.1). UCB achieves higher average reward.](./assets/images/figure_2_4.png)

**Results**: UCB consistently outperforms $\epsilon$-greedy, especially in the first few hundred steps where directed exploration matters most. Its limitation: harder to extend to full RL with large state spaces and non-stationary problems.
